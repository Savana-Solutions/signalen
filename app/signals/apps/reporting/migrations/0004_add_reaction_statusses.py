...
CREATE VIEW "signals_ext_tdo" AS
    SELECT
        ...
        CASE
           WHEN stat.state = 'm' THEN 'Reported'
           WHEN stat.state = 'i' THEN 'Awaiting handling'
           WHEN stat.state = 'b' THEN 'In progress'
           WHEN stat.state = 'h' THEN 'On hold'
           WHEN stat.state = 'planned' THEN 'Planned'
           WHEN stat.state = 'ready to send' THEN 'To send naar extern systeem'
           WHEN stat.state = 'o' THEN 'Completed'
           WHEN stat.state = 'a' THEN 'Cancelled'
           WHEN stat.state = 'reopenend' THEN 'Reopened'
           WHEN stat.state = 'reopened' THEN 'Reopened'
           WHEN stat.state = 's' THEN 'Split'
           ...
           WHEN stat.state = 'reopen requested' THEN 'Request to reopen'
           WHEN stat.state = 'reaction requested' THEN 'Reaction requested'
           WHEN stat.state = 'reaction received' THEN 'Reaction received'
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('reporting', '0003_added_status_display_to_dro_view'),
    ]
