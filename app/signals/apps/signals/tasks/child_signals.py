...
from signals.apps.signals.workflow import COMPLETED, CANCELLED
...
def update_status_children_based_on_parent(signal_id):
    ...
    if signal.is_parent and signal.status.state in [COMPLETED, CANCELLED, ]:
        text = 'Hoofdmelding is afgehandeld'

        # Lets check the children
        children = signal.children.exclude(status__state__in=[COMPLETED, CANCELLED, ])
        for child in children:
            # All children must get the state "CANCELLED"
            data = dict(state=CANCELLED, text=text)
