...
from signals.apps.signals.workflow import TO_SEND
...
class TestSignalReceivers(TestCase):
    ...
    def test_status_update_handler(self, mocked_tasks: Any) -> None:
        ...
        TO_SEND state with TARGET_API_SIGMAX target API, the push_to_sigmax
        ...
        new_status = StatusFactory.create(_signal=signal, state=TO_SEND, target_api=Status.TARGET_API_SIGMAX)
    ...
    def test_status_update_handler_not_called(self, mocked_tasks: Any) -> None:
        ...
        This test case ensures that when the status of a signal is updated to a
        non-TO_SEND state or a non-TARGET_API_SIGMAX target API, the push_to_sigmax
        ...
        new_status = StatusFactory.create(_signal=signal, state=TO_SEND, target_api=Status.TARGET_API_GISIB)
        signal.status = new_status
        signal.save()
