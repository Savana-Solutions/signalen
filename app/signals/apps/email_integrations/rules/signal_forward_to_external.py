...
from signals.apps.signals.workflow import FORWARDED_TO_EXTERN
...
class ForwardToExternalRule(AbstractRule):
    def validate(self, signal, status):
        ...
        - The status state must be FORWARDED_TO_EXTERN
    ...
    def _validate_status(self, status):
        """
        Validate that the status is FORWARDED_TO_EXTERN
        """
        return status.state == FORWARDED_TO_EXTERN
